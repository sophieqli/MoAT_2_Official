...
def train_model(
    ...
    """
    Train model with optional contracted dataset (for moat contraction).
    If alpha is None -> standard training.
    If alpha in (0,1) -> blends between original and contracted likelihoods.
    """
    ...
    # if contracted datasets provided
    if train_contracted is not None:
        train_loader_contracted = DataLoader(dataset=train_contracted, batch_size=batch_size, shuffle=True)
        valid_loader_contracted = DataLoader(dataset=valid_contracted, batch_size=batch_size, shuffle=True)
        test_loader_contracted = DataLoader(dataset=test_contracted, batch_size=batch_size, shuffle=True)
    else: 
        train_loader_contracted, valid_loader_contracted, test_loader_contracted = None,None, None
    ...
    for epoch in range(max_epoch):
        ...
        if alpha is not None:
            ...
            #marg_consistency_diagnostics(model.V_orig, model.E_orig)
            marg_consistency_diagnostics(model.V_compress, model.E_compress[0])
...
def main():
    ...
    print("train_0.x shape:", train.x.shape)
    ...
    model.contract_and_delete(xi, xj, train.x)
